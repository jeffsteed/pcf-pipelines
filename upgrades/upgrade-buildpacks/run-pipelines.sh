...
fly -t $CONCOURSE_TARGET set-pipeline -n -p upgrade-buildpacks-${ENVIRONMENT_NAME} --config ./${ENVIRONMENT_NAME}/pipeline.yml --load-vars-from $TEMP_DIR/params.yml


fly -t $CONCOURSE_TARGET unpause-pipeline -p upgrade-buildpacks-${ENVIRONMENT_NAME}
